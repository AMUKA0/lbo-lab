...
- **The answers ship alongside the formulas.** openpyxl writes `<f>=…</f>` and nothing else, because it cannot calculate. Excel fills those in on open — the iteration flag is set in the file — but anything that *renders* rather than *calculates* shows every computed cell blank: Explorer and Quick Look previews, Gmail, Drive, Slack, Teams, GitHub. On the Model sheet that was 259 of 326 cells, so the file someone previewed before opening it looked broken. Every formula cell now carries a cached result taken from the engine, and three tests keep it honest: none may be missing, no cell may carry **two** value elements, and on the acyclic convention every cached value must equal what an independent evaluator computes from the formula beside it. The middle one exists because the first attempt shipped broken: openpyxl writes an empty `<v/>` placeholder after each formula, a guard looking for `"<v>"` did not see it, and the injection appended a second value. That is well-formed XML — so a strict parse passed, and so did openpyxl, which takes the first value — and it is invalid OOXML, so Excel discarded every sheet it touched. Well-formed is not valid, and every tool reachable from Python tests the wrong one. Evaluating the workbook instead was measured and rejected — the interest circularity leaves only 27% resolvable without iteration, and iterating takes 24 seconds.
...
- **Hilton needs its restructuring, and reproduces the outcome with it.** Strip the 2010 events out and the structure runs out of liquidity in year three — the model locating the crisis from the numbers alone, in the exact year Blackstone renegotiated. Model that renegotiation as reported (~$2bn of debt repurchased for ~$800m, ~$2bn converted to preferred) and the deal runs the full eleven years and returns **3.01× against a reported ~3.0×**. A test pins both halves, because either alone is a much weaker claim.
...
| **Interest deductibility** | **§163(j)**: business interest is deductible only up to **30% of adjusted taxable income**. Disallowed interest is still paid — it simply stops sheltering income, so a levered structure can post a book loss and owe cash tax. It carries forward **indefinitely** and competes with the next year's own interest for the same capacity. Cash coupon, PIK accrual and fee amortisation (OID) are all inside the cap; the undrawn commitment fee is outside it, per the 2020 final regulations. ATI is EBIT-like under current law, with a switch for the EBITDA-like basis that applied to years beginning before 2022. On by default and switched off for every pre-2018 case study, where the cap would be an anachronism. |
| **Taxes** | Charged on taxable income — book EBT plus back whatever §163(j) refused — in that order, because that is the order of the statute. Losses carry forward as **NOLs and shelter up to 80% of later taxable income** (post-TCJA §172(a)); the limitation is configurable. |
...
| **Sponsor support** | Follow-on capital mid-hold, in one mechanic covering all three shapes a real rescue takes: cash only (an equity cure), cash plus a larger face value (a repurchase below par — Blackstone bought ~$2bn of Hilton's debt for ~$800m in 2010), or face value with no cash (a debt-for-equity conversion). It funds the year it goes into, retires debt senior-first, lands in the IRR vector as an outflow, and raises the invested-capital denominator — so a rescued deal shows a worse multiple than one that never needed rescuing. |
